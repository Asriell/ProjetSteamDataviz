...
function display_graph1(svg_already_exists, svg, change = undefined) {
    ...
    d3.json(urlplayersjson).then((json) => {
        ...
        if (!svg_already_exists) {
            ...
            svg1.selectAll(".abscisses").transition().duration(1000).call(x_axis).selectAll("text").style("text-anchor", "start").attr("dx", "-.8em").attr("dy", ".15em")
            .attr("transform", function (d) {
                return "rotate(-30)";
            });
        ...
        svg1.selectAll(".ordonnees").selectAll(".tick").select("line").attr("stroke",axisColor);
        svg1.selectAll(".ordonnees").selectAll(".tick").select("text").attr("fill",axisColor);
